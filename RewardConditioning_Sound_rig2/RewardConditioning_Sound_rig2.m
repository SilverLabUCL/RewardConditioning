...
%     if isrig2, TrigWait = 5; else, TrigWait = 0.1; end   % wait for trigger time, else just pre-stim
    ...
    for currentTrial = 1 : MaxTrials
        ...
        if isrig2, io.LickInput = 'BNC2High'; else, io.LickInput = 'Port1In'; end              % lick touch
        ...
        %  when sensor is touched, release 50 drops every 0.5 seconds
                ...
                sma = AddState(sma, 'Name', 'TrigTrialStart', ...                    
                    'Timer', interval, ...
                    'StateChangeConditions', {io.LickInput, 'Drop1_Openvalve', 'Tup', 'TrialEnd'}, ...
                    'OutputActions', {});
        ...
        if doReward 
            ...
            DelayTup = lastState; LickAction = 'StopLicking';
        end
        ...
        sma = AddState(sma, 'Name', 'TrigTrialStart', ...                    % wait for trigger
                'Timer', TrigWait, ...
                 'StateChangeCondition',{io.TrialTrigger, 'PreStimulus', 'Tup','PreStimulus' }, ...
                'OutputActions', [ io.CameraTrig]);
        ...
        sma = AddState(sma, 'Name', 'WaitForLick', ...
                    'Timer', 300,...
                    'StateChangeConditions', {io.LickInput, 'Reward', 'Tup', 'NoResponse'}, ...
                    'OutputActions', {});
        
        sma = AddBitcode(sma, currentTrial, io.Bitcode, [ io.CameraTrig], 'SamplePeriod');

        sma = AddState(sma, 'Name', 'SamplePeriod', ...                         % play sound
            'Timer', S.GUI.SamplePeriod, ...
            'StateChangeConditions', {'Tup',SampleTup}, ...
            'OutputActions', [stimOut  io.CameraTrig]);   
        ...
        sma = AddState(sma, 'Name', 'Delay_nolick', ...                         
            'Timer', delay, ...
            'StateChangeConditions', {'Tup', DelayTup, io.LickInput, 'Delay_nolick2'}, ... % move if lick
            'OutputActions', [DelayTrig  io.CameraTrig]);     % start delay timer and wait...

        sma = AddState(sma, 'Name', 'Delay_nolick1', ...                         % wait ...
            'Timer', delay, ...
            'StateChangeConditions', {'Tup', DelayTup, 'GlobalTimer1_End', DelayTup, io.LickInput, 'Delay_nolick2'}, ... % move if lick
            'OutputActions', [ io.CameraTrig]);                   
        
        
        sma = AddState(sma, 'Name', 'Delay_nolick2', ...                         % wait for 3rd lick
            'Timer', lickDelay, ...
            'StateChangeConditions', {'Tup', 'Delay_nolick1', 'GlobalTimer1_End', DelayTup, io.LickInput, 'Delay_nolick3'}, ... % move if lick
            'OutputActions', [ io.CameraTrig]);                           
        
        
        sma = AddState(sma, 'Name', 'Delay_nolick3', ...                         % wait for 3rd lick
            'Timer', lickDelay, ...
            'StateChangeConditions', {'Tup', 'Delay_nolick1', 'GlobalTimer1_End', DelayTup, io.LickInput, EarlyLickAction}, ... % punish if licking
            'OutputActions', [ io.CameraTrig]);                   
        
        sma = AddState(sma, 'Name', 'Punish', ...                         % punish sound
            'Timer', S.GUI.SamplePeriod, ...
            'StateChangeConditions', {'Tup', 'StopLicking'}, ... %
            'OutputActions', [io.Punish  io.CameraTrig]);                   
        ...
        sma = AddState(sma, 'Name', 'AnswerPeriod', ...                         % pole still in and wait for response
            ...
            'StateChangeConditions', {io.LickInput, LickAction, 'Tup', AnswerTup}, ...
            'OutputActions', [ io.CameraTrig]);

        sma = AddState(sma, 'Name', 'Reward', ...                               % turn on water
            'Timer', S.GUI.WaterValveTime, ...
            'StateChangeConditions', {'Tup', 'RewardConsumption'}, ...
            'OutputActions', [io.WaterOutput  io.CameraTrig]);
        ...
        sma = AddState(sma, 'Name', 'Baseline2', ...                          % post-stim/reward baseline before next trial
        ...
        sma = AddState(sma, 'Name', 'StopLicking', ...                          % stop licking before advancing to next trial
            'Timer', S.GUI.StopLickingPeriod, ...
            'StateChangeConditions', {io.LickInput, 'StopLickingReturn', 'Tup', 'TrialEnd'}, ...
            'OutputActions', [ io.CameraTrig]);
        ...
        if ~isempty(fieldnames(RawEvents)) % If trial data was returned
            ...
            if S.GUI.ProtocolType > 2
                UpdatePerfOutcomePlot(TrialTypes, BpodSystem.Data,  io.LickInput);
                TrialBehPlot(BpodSystem.GUIHandles.LickFig, BpodSystem.Data.nTrials, currentTrial, io.LickInput)
            end
...
%%
%%
function UpdatePerfOutcomePlot(TrialTypes, Data, lickevent)

    global BpodSystem
    Outcomes = zeros(1,Data.nTrials);
    if  nargin<3, lickevent = 'Port1In'; end
    for x = 1:Data.nTrials
        ...
        if Data.TrialSettings(x).GUI.ProtocolType > 2        
            if ~isnan(Data.RawEvents.Trial{x}.States.Reward(1))
                ...
                if isfield( BpodSystem.Data.RawEvents.Trial{x}.Events, lickevent)
                   if any(BpodSystem.Data.RawEvents.Trial{x}.Events.(lickevent)>=BpodSystem.Data.RawEvents.Trial{x}.States.Reward(1) ... 
                        & BpodSystem.Data.RawEvents.Trial{x}.Events.(lickevent)<=BpodSystem.Data.RawEvents.Trial{x}.States.RewardConsumption(2))
                ...
                if isfield( BpodSystem.Data.RawEvents.Trial{x}.Events, lickevent )
                   bad_licks = find( BpodSystem.Data.RawEvents.Trial{x}.Events.(lickevent)>=BpodSystem.Data.RawEvents.Trial{x}.States.Delay(2)); 
